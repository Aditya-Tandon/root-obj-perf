...
def get_purity_mask_one_one(gen_particles, reco_objects, CONFIG=None):
    ...
    for i in range(len(gen_particles)):
        ...
        for reco_idx in range(len(reco_vec)):
            ...
            if (
                min_delta_r < CONFIG["matching_cone_size"]
                and min_gen_idx not in used_gen_indices
            ):
...
def get_purity_mask_cross_matched(
    gen_particles, reco_objects, CONFIG=None, return_gen_idx=False
):
    ...
    dr_matrix_reco_major = reco_objects[:, :, None].vector.deltaR(
        gen_particles[:, None, :].vector
    )
    ...
    min_dr_reco_major = ak.fill_none(
        ak.min(dr_matrix_reco_major, axis=2), np.inf
    )  # events, n_reco

    dr_matrix_gen_major = gen_particles[:, :, None].vector.deltaR(
        reco_objects[:, None, :].vector
    )
    ...
    pure_cross_matched = (ak.fill_none(back_check_reco_idx, -1) == reco_idxs) & (
...
def calculate_pur_eff_vs_variable(
    gen_particles, reco_objects, mask, variable, bins, is_purity_plot=False
):
    """
    ...
    frac_offline = np.divide(
        h_matched, h_total, out=np.zeros_like(h_total, dtype=float), where=h_total != 0
    )
    err_offline = np.sqrt(
        ((h_matched + 1) * (h_total - h_matched + 1))
        / ((h_total + 2) ** 2 * (h_total + 3))
    )  # Ullrich and Xu

    return frac_offline, err_offline


def calculate_roc_points(reco_jets, is_pure_mask, tagger_name, weights=None):
    """
    ...
    sig_weights = (
        ak.to_numpy(ak.flatten(weights[is_pure_mask])) if weights is not None else None
    )
...
def get_pure_jet_idxs_cross_matched(gen_particles, reco_objects, CONFIG=None):
    ...
    is_mutual_match = (back_check_idx == gen_indices) & (
